...
./gradlew build          # → build/libs/aqualumen-forge-1.20.1-0.3.1-alpha.jar
./gradlew runClient      # локальная проверка интерфейса
python3 tools/gen_item_texture.py   # перегенерировать иконку предмета
```

Первая сборка требует интернета (ForgeGradle скачивает MDK и маппинги). Сборка `0.3.1-alpha` проходит через `./gradlew build` (Java 21 toolchain used locally).
...
- **Проверка результата.** `build/libs/aqualumen-forge-1.20.1-0.3.1-alpha.jar` — в `mods/` на сервере и в клиентскую сборку. `./gradlew runClient` поднимает клиент с модом без ручной установки.
