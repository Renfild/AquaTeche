...
(() => {
  ...
  const tabMeta = {
    ...
    privileges:["Привилегии",'<svg viewBox="0 0 24 24"><path d="M12 3l2.4 5.1 5.6.7-4.1 3.9 1 5.6L12 15.6 7.1 18.3l1-5.6L4 8.8l5.6-.7L12 3Z"/></svg>'],
    ...
    atlas:["Атлас",'<svg viewBox="0 0 24 24"><path d="M5 4.5A2.5 2.5 0 0 1 7.5 2H19v17H7.5A2.5 2.5 0 0 0 5 21.5Z"/><path d="M5 4.5v17"/><path d="M9 6.5h6M9 10h5"/></svg>'],
  ...
  function storeView(s, ranksOnly) {
    ...
    const cards = offers.map((o, cardIndex) => {
      ...
      if (!meta) {
        const itemId = SHOP_ICONS[o.id] || "";
        const ns = itemId.includes(":") ? itemId.split(":")[0] : "";
        const shopCat = ns === "ae2" ? "ae2" : (ns === "industrialupgrade" ? "industrialupgrade" : (ns === "botania" ? "botania" : "other"));
        const key = esc((o.title + " " + o.subtitle).toLowerCase());
        return `<article class="card offer${o.owned ? " is-owned" : ""}" data-shop-name="${key}" data-shop-ns="${esc(shopCat)}" style="--i:${cardIndex}">
          ${o.owned ? `<span class="offer-badge">куплено</span>` : ""}
          <div class="offer-art">${offerIcon(o)}</div>
          <h3>${esc(o.title)}</h3>
          <p>${esc(o.subtitle)}</p>
          <div class="offer-foot">
            <span class="price">${o.currency === "gems" ? (num(o.price) + " гем.") : coins(o.price)}</span>
      ...
      return `<article class="card offer rank-card${o.owned ? " is-owned" : ""}" style="--rc:${c};--i:${cardIndex}">
        ${o.owned ? `<span class="offer-badge">активно</span>` : ""}
        <div class="rank-glyph" style="color:${c};background:${c}14">${rankGlyph(o.id, c)}</div>
        <h3>${esc(o.title)}</h3>
        <p>${esc(o.subtitle)}</p>
        <div class="offer-foot">
          <span class="price">${o.currency === "gems" ? (num(o.price) + " гем.") : coins(o.price)}</span>
          ${o.owned ? `<span class="owned-tag">уже есть</span>` : `<button class="button primary rank-more" data-id="${esc(o.id)}">Подробнее</button>`}
        </div>
      </article>`;
    }).join("");
    ...
    const filterBar = ranksOnly
      ...
      : `<div class="shop-filter">
          ...
          <div class="shop-chips" id="shopChips">
            <button class="atlas-pill active" data-shop="all">Все</button>
            ...
            <button class="atlas-pill" data-shop="other">Прочее</button>
          </div>
        </div>`;
  ...
  function eventsView(s) {
    ...
    return `<div class="view">${title("События океана", banner)}
      <div class="store-grid">${cards || '<div class="empty">Контракты появятся после перезапуска сервера СЃ модом aquatech_ui</div>'}</div>
    </div>`;
  }
  ...
  function bindViewActions() {
    ...
    // Магазин: поиск + фильтр по источнику (AE2 / IU / Botania / прочее)
  ...
  window.AquaLumen = {
    // Закрывает верхний открытый слой (его вызывает мод по Esc). true = что-то закрыли.
    escapeLayer() {
