...
| **2026-08-26** | `v2.8.5` | ZCode | **Квесты по эпохам + пак 2.9.252 + лаунчер 2.9.80.** FTB: глава 1 → пар → базовая электрика → улучшенная; карьеры/космос optional (крафты выключены). KubeJS: магазин Кота парсится, Infinity-крафты Avaritia на месте, sky/alpha удочки без фантома и астероида. `/start` на весь экран. `publish_client_pack` берёт свежие aquatech_ui/aqualumen банки. SFTP `--only` без mods/ не пилит first-party jars. | `config/ftbquests/`, `kubejs/server_scripts/`, `docs/start.html`, `tools/publish_client_pack.py`, `scripts/tasks/deploy_apexnodes_sftp.py` |
