...
| **2026-09-07** | `v2.8.47` | ZCode AI | **Критический фикс синка лаунчера (pack CDN отдаёт 404/307 на контент):** после ночного передеплоя воркера `/pack/*` (кроме manifest.json) отдавал 404, а `hub.html` — 307 от `html_handling=auto-trailing-slash` на несуществующий таргет; лаунчер падал «Синхронизация не удалась (2 ошибок)» и блокировал запуск. Фикс в `worker/index.js` `/pack/`-ветке: серверный рерайт `.html` на extensionless-ассет (лаунчер больше не зависит от редиректов), `cache-control` раздельный (jar — immutable, мутируемые hub.html/kits.json — max-age=60, манифест — 15), 404 — `no-store` (плохие ответы больше не залипают в edge-кэше). Верификация: все 8 store-URL манифеста 2.9.298 отдают 200 с совпадающим md5, портал-смоук зелёный. **Сопутствующий фикс лаунчера (в дереве, релиз после обновления .gh_token):** `PurgeExtras` больше не чистит `config/` — та затирала рантайм-данные игрока (история JEI, скины easy_npc, −327 файлов за синк); полный reconcile остался для `mods/kubejs/resourcepacks`. | `worker/index.js`, `launcher/src/AquaTechLauncher.Core/LauncherConstants.cs`, `ManifestSync.cs` |
