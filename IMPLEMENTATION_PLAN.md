...
| **2026-09-07** | `v2.8.50` | ZCode AI | **F4-магазин: ранговые карточки + модалка покупки по образцу McSkill (aqualumen 0.3.34, пак 2.9.301):** изучены референсы — сайт mcskill.net (скриншоты в `scratch/referenc_mcskill/`) и их игровой магазин `nextshop` из разбора HiTech1211 (карточки 285×385 с фонами по редкости + книжки 205×205). Наш вариант в тёмном океан-стиле: ранги рендерятся стеклянными карточками с якорным крестом-силуэтом, окрашенным в цвет ранга (МОРЯК aqua, ШКИПЕР blue, КАПИТАН gold, АДМИРАЛ orange, ЛЕГЕНДА purple, VIP red), цветной пилл префикса, цветная рамка/бейдж; «Подробнее» открывает модалку с гербом, префиксом, чек-листом возможностей и покупкой (`store.buy`). Перки живут в `RANK_META` внутри hub.html — без изменений протокола/сервера. Не-ранговые товары (гемы, БП) сохранили прежний вид. Проверено превью в браузере на тестовом снапшоте (скриншоты в референс-папке), деплой 2.9.301 с md5-верификацией CDN, сервер на 0.3.34, smoke зелёный. | `tools/build_hub_html.py`, `config/aqualumen/hub.html` (+8 копий), `mods/aqualumen-ui/gradle.properties` |
