...
| **2026-08-27** | `v2.8.7` | ZCode | **Лаунчер 2.9.81:** ник с сервера (`IN_GAME_UNREGISTERED`) больше не маскируется под «неверный пароль». `GET /api/auth/nick`, `code: unclaimed` на логине. DPAPI: при сбое Protect сессия не пишется plaintext. | `PortalApi.cs`, `HttpDownload.cs`, `SessionStore.cs`, `MainViewModel.cs`, `functions/api/auth/nick.js`, `functions/api/login.js` |
